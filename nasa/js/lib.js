var youtubeGallery = function(element, user, size){
	...
	$.ajax({
		url:'http://gdata.youtube.com/feeds/api/videos?max-results='+size+'&alt=json&orderby=published&format=5&author='+user+'&callback=?',
		type: 'GET',
		dataType: 'json',
...
var player = function(videoId, autoplay){
	return [
		'<iframe width="548" height="348" src="http://www.youtube.com/embed/'+videoId+'?autoplay='+autoplay+'" frameborder="0" allowfullscreen></iframe>'
	].join('');	
}
...
$(document).ready(youtubeGallery('#gallery','NASAtelevision', 4));